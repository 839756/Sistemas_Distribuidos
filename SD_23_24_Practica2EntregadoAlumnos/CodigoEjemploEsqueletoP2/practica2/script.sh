#!/bin/bash
    ...
    gnome-terminal --working-directory=$(pwd)/cmd/escritor -- bash -c "go run main.go 2; read -p 'Presione [Enter] para cerrar esta terminal...'"

    # Darle tiempo al programa escritor para que termine de ejecutar
    sleep 4

    # Comparar el contenido de fichero_1.txt y fichero_2.txt
    if cmp -s "cmd/lector/fichero_1.txt" "cmd/escritor/fichero_2.txt" ; then
